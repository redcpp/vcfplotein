...
import Colorgorical from '@/lib/json/colorgorical.json'
import ColorsGoogle10 from '@/lib/json/colorsGoogle10.json'
...
export default class Extractor {
  ...
  parseInfo (data, selectedTranscript) {
    ...
    for (const [, t] of data.Transcript.entries()) {
  ...
  _assertProperties (obj, keys) {
    for (const key of keys) {
      assert(Object.prototype.hasOwnProperty.call(obj, key),
        `Extractor: ${key} not found in ${Object.keys(obj)}`)
    }
  }
  ...
  _getColor (name) {
    if (!Object.prototype.hasOwnProperty.call(this.colorOf, name)) {
