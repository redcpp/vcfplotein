...
- **Lighter bundle.** The ~14 MB of gene-coordinate JSON moved out of the build into runtime fetches of plain `.json`; only the genome build a dataset needs is downloaded.
- **Resilient annotation.** Ensembl VEP is queried in bounded-concurrency batches with exponential-backoff retry on rate-limit responses, and companion-database lookups degrade gracefully when the upstream is unavailable.
...
1. **Upload & parse** — Accepts `.vcf`, `.vcf.gz`, or saved `.json` bookmarks directly in the browser. Large files are streamed and parsed in a background worker, so even multi-gigabyte VCFs load without freezing the interface.
